...
- The Heads-Down Display's squad comm boxes read stand-in state: a box is captioned with the machine's type name where retail names the pilot from the player's save, and its OBJECTIVE: line shows the last order transmitted to that slot where retail shows the AI state the pilot is actually in. A transmitted order is recorded against the slot and nothing else, there being no squad AI to receive it. See "Engine coverage" in [`docs/formats/heads-down-display.md`](docs/formats/heads-down-display.md#engine-coverage).
