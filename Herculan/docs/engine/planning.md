...
Implementation history (what shipped, when, and why) is **not** kept here — it lives in git log and the per-topic docs under `docs/simulation/` and `docs/formats/`, which are the canonical reference for any given subsystem's reverse-engineering and porting detail.
