...
- `ShellMap_LoadScriptDat` (`004243d7`, VSHELL, `shellmap.cpp`), called from `ShellMap_Constructor` (`00423f43`) — VSHELL's map-editor reader, independent of DBSIM's. Same 13-block structure and order; keeps a different subset per block (see per-block table). Confirms record shape/strides via a second, independently compiled reader.
