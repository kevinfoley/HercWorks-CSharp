...
It is driven by the bay selection rather than by screen entry: `FUN_0043d64d` calls it when the selected bay changes, and `Repair_Enter` reaches it only when the bay it opens on holds nothing it can work on. The builder's own construction-time indices are the walker set, `0x4e + row` and `0x54 + row`.
...
Row 0 is the player; rows 1-3 are the three squad positions. `FUN_00441b08` fills the pointers at `004776e0` with the squad member whose position, pilot record `+0x27`, is the row — `FUN_004102d6` tests the three in pointer order — or null. `FUN_00441c4f` fills the values: the player's name, `0x35 + skill` and the chassis name `0x6e + type` of the player's bay (`00482a9e`), or `0x7e` `None` when that bay is empty; the same three from a squad member's record and bay; and for a row with no pilot, no portrait and `estext.bin` entry 0 in all three.

`FUN_00441857` colours each row by whether it is below `DAT_00482a78`, the count of squad positions in play ([`../formats/save-games.md`](../formats/save-games.md#savgame_sav--block-order)):
...
**The scope paints, and its paint is what makes the tab screens black.** Its handler sends event 4 to `FUN_0040cb40`, which fills the whole scope rect, `{0, 0x1e, 0x27f, 0x1df}`, with `0x10`, and the show posts that event. Tabs 2-7 all install their palette through the scope (`0043b162` cases 2-7 call `00439da0`), and nothing repaints the backdrop-textured root afterwards, so each of those tabs draws its screen over a black canvas and shows black wherever its widgets leave it bare. The main menu and the save screen go through the scope too, then put up their own backdrop-textured root over the fill, which is why they show the bay.
...
| The palette scope draws nothing — it exists only to fire the palette install | It carries no bitmap, no caption and no chrome, and its handler's event 2 is the install. Its event 4 is a paint, `FUN_0040cb40`, which fills its rect with `0x10`; that fill is why retail's tab screens are black ([The palette](#the-palette)) |
...
- **Unported:** the squad panel on WEAPONS and BUILD, which goes with those two screens, and their arms of `FUN_0043d64d`.
- **Unported:** every click on the crew screen — selecting a row (`FUN_00441b85`), a squad portrait's assignment (`FUN_00441eb8`), a roster click's assignment (`FUN_00442055`) and `CLEAR` (`FUN_00441f94`).
...
- **Open:** what retail draws for a machine under construction whose body bank lacks the construction frames ([The bay picture](#the-bay-picture)). `FUN_00414e5b` indexes past them unchecked; the engine draws nothing for a missing frame.
