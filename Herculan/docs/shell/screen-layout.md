...
`ESAlert_Ctor` is `TitledPanel_Ctor` with its own vtable, the mouse bits ORed into the event mask and a buffer the size of its rect; its paint is `TitledPanel_Paint`. Both frames keep the class's `0x25` checkerboard.
...
**The weapons screen is drawn and fits**, from the same save's armory stock and mounts, `gam\arm_weap.dat`, `dba\arm_weap.dba`, `wpn_desc.bin`, `gam\arm_hots.dat` and the `_out` banks. `ShellWeaponsScreen` places every widget above, lists the 27 rows with their counts, unlock gaps and dead rows, shows the lit row's picture and description, puts the guidance buttons up for a missile rack and shows a kind's picture from them, and opens on the first row. The hotspots over the bay picture and both steppers select a hardpoint and outline its socket, a row click then fits its weapon as [Fitting a weapon](#fitting-a-weapon) says, and a guidance button writes its kind into the mount. `ShellHangar` keeps the armory's stock as the units themselves, each weapon's list in the save's order with its head last, and a mount holds the unit fitted in it, so fitting, stripping a scrapped machine and refitting move units rather than counts. The roster click takes the arming arm's bay rule. Its bay starts from the repair screen's, as the crew and build screens' do, and with no bay selected the steppers do nothing, where the original reads through `00482abf` — this engine's choice.

**The build screen is drawn and selects**, from the same save and `gam\herc_inf.dat`. `ShellBuildScreen` places every widget above, gates the rows on the save's availability flags, prints the selected chassis's figures and the net salvage, and gates `SCRAP` and `BUILD` on the bay; `ShellRepairDiagrams` draws the blueprint from the repair screen's layouts. A row click moves the chassis and a roster click moves the bay, as [The build screen](#the-build-screen) says. `BUILD` orders the chassis into the bay through `ShellHangar.Order`, and `SCRAP` puts up the scrap dialog. Its bay starts from the repair screen's, as the crew screen's does, and with no bay selected it greys both buttons where the original reads past the array — this engine's choice.

**The scrap dialog is drawn and scraps.** `ShellScrapDialog` places every widget of [The scrap dialog](#the-scrap-dialog) over whichever of the two tabs opened it and quotes `ShellRepairCosts.ScrapValue`; `ACCEPT` runs `ShellHangar.Scrap`, which credits the pool, returns the mounts at 80 or better to stock, empties the bay and unassigns its pilot, and the repair tab then moves to the first finished machine. While the dialog is up nothing else is hit-tested, which is this engine's choice ([Open](#open)).
