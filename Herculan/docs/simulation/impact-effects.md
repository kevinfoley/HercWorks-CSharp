...
Every `PROJ.DAT` record carries three four-entry `ImpactFX` arrays. The shot record's `+0x0a` (see [`weapon-firing.md`](weapon-firing.md#the-shot-record)) points at all three as one twelve-entry `short` array, indexed `group * 4 + (rand & 3)` — so all four ids in a group are equally likely and the file's own field order is the group order.
