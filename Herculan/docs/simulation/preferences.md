...
- **Five of the nine settings are stepped, saved and then read by nothing.** TERRAIN DISTANCE (through `Terrain.TerrainDetail`), TERRAIN TEXTURE (the terrain item's texture binding, re-read every frame), EFFECTS DETAIL (`SoundDirector.DetailSetting`, which is the row's audio half and all this engine knows of it) and the twelve control bindings (through `Input.JoystickBindings`) have a consumer. MUSIC, SOUNDS, PILOT MESSAGE, COMPUTER MESSAGE and HERC DETAIL round-trip the file correctly and change nothing on screen or in the mix; STRUCTURE DETAIL has nothing to change yet, the engine drawing `TSDetailPart`'s finest level unconditionally.
