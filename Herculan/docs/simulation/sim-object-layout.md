...
| A plausible code address after a vtable's last slot is a 35th slot | `ES2DumpVtable` resolves and disassembles any valid address. The word past the end is usually the next vtable block's class-record pointer ([`../formats/borland-rtti.md`](../formats/borland-rtti.md#vtable-block)): `0046b7c8` and `0040c3d8` both look like code and are neither functions nor slots. Check for a prologue *and* for a real call site. |
