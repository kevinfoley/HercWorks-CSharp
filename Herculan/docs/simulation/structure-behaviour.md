...
- **Nothing draws the turret moving.** The simulation poses the nodes correctly and the shots leave the right muzzle point, but the renderer only splits a model into per-node segments for a `MechObject`, and a structure's model is split by cell instead — a split it also needs, for its damage states. Drawing both at once is render work that has not been done, so a tower tracks and fires without visibly turning, and a radar mast's dish does not sweep.
