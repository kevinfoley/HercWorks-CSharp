...
`AnimationThread.InterpolateSeekPosition` is the switch, and clearing it restores the original's arithmetic exactly. It defaults **on** — the deliberate exception to this engine's retail-by-default rule — and is the hook for the Tweaks menu. Playback is untouched: only a seek produces a remainder, and the locomotion thread is never seeked.
