...
/// <c>FUN_00441857</c>, matched to pilots by <c>FUN_00441b08</c>, selected by <c>FUN_00441b85</c> and
/// filled by <c>FUN_00441c4f</c>. See docs/shell/screen-layout.md, "The crew screen".
///
...
public sealed class ShellCrewScreen {
	...
	/// occupied bay in turn to <c>FUN_0043d64d</c>, and the crew arm accepts each finished one — so the
	...
	/// on rows 1-3 the squad member whose position is that row (<c>FUN_00441b08</c>), or null.
	/// </summary>
	...
	/// <summary>
	/// <c>FUN_00441b85</c> without its repaints: moves the bay to the row's pilot's — the player's own
	...
	/// <summary><c>FUN_0043d64d</c>'s crew arm, with the assignment it makes on a roster click left out.</summary>
	...
	/// portrait and three label/value pairs. Rows inside the squad's positions (<c>FUN_00441857</c>)
	/// take a <c>0x25</c> body, greyer labels and value backing to match; the rest keep the shell's
	/// background. The values are drawn in <c>0x29</c> whichever: <c>FUN_00441857</c> writes them
	/// <c>0x16</c> or <c>0x17</c>, and <c>FUN_00441c4f</c>'s <c>Text_SetString</c> then overwrites that
	...
	private void PaintRow(ShellSurface surface, HudFont? font, ShellText? text, int row) {
		...
		// FUN_00441c4f: a row with no pilot has all three values set to estext.bin entry 0, the empty
	...
	/// <summary><c>FUN_00441857</c>'s two sets: a row inside the squad's positions, and one outside them.</summary>
