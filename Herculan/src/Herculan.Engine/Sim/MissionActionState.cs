...
/// record — the <b>fired flag</b> at <c>+0x0a</c>, zeroed at load and set once, for good, by
/// <c>Action_Fire</c> (<c>00423430</c>).
///
...
/// current order (<see cref="MissionGroup.AiTick"/>). Both only ever ask <see cref="Fired"/>.</para>
/// </summary>
public sealed class MissionActionState {
	...
	/// <summary>
	/// <c>action+0x0a</c> — whether the action has fired. One-shot: <c>Action_TestTrigger</c>
	/// (<c>004234b8</c>) answers "in the area" for an action that has already fired without testing
	/// anything, and <c>Action_Fire</c> does nothing a second time.
	/// </summary>
	...
	/// areas in turn and fires on the first that catches it.
	/// </summary>
	/// <returns>
	/// Whether the action is now fired, which includes an action that already was — the original
	...
	/// <summary>
	/// <c>Action_Fire</c> (<c>00423430</c>) — latches the flag and applies the action's counter
	/// operations.
	///
	...
	/// </summary>
	// TODO: Consider renaming, as "Fire" is easily confused with firing weapons.
	public void Fire(SimWorld world) {
		if (Fired) {
			return;
		}
	...
	/// count stands in for the posts — see <see cref="Fire"/>.
	/// </summary>
